...
export default class CognitoSRP {
  ...
  constructor(config: { userPoolName: string; a?: string; A?: string }) {
    this.userPoolName = config.userPoolName;

    if (config.a || config.A) {
      if (!config.a || !config.A) throw new Error('Both "a" and "A" are required if supplying one.');

      this.a = BigInt(`0x${config.a}`);
      this.A = BigInt(`0x${config.A}`);
    }
